...
if [ "$1" = '/opt/actions-runner/run.sh' ]; then
  ...
  /opt/actions-runner/config.sh --unattended --url "https://github.com/${REPOSITORY}" --token "$REPOSITORY_TOKEN" --disableupdate --replace --name "$RUNNER_NAME" --labels "gpu" --labels "dind"
